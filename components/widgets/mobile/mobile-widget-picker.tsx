...
const mobileWidgets: Record<WidgetType, {
  id: WidgetType
  name: string
  description: string
...
export function MobileWidgetPicker({
  ...
  const handleWidgetSelect = useCallback((widget: typeof mobileWidgets[keyof typeof mobileWidgets]) => {
    if (!canAddMoreWidgets) return
    onWidgetSelect(widget.id)
  }, [canAddMoreWidgets, onWidgetSelect])

  // Widget card component
  const WidgetCard = ({ widget }: { widget: typeof mobileWidgets[keyof typeof mobileWidgets] }) => {
    ...
    return (
      ...
      >
        <Card className={cn(
          ...
          !canAddMoreWidgets && 'opacity-50 cursor-not-allowed',
          ...
          <CardHeader className="pb-3">
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                <div className={cn(
                  'p-2 rounded-lg',
                  categoryInfo.color.replace('text-', 'bg-').replace('-800', '-100')
                )}>
                  <IconComponent className={cn(
                    'h-5 w-5',
                    categoryInfo.color.replace('bg-', 'text-').replace('-100', '-600')
                  )} />
                </div>
                <div>
          ...
          <CardContent className="pt-0">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex items-center gap-1">
                ...
                <span className="text-xs text-gray-500">{widget.popularity}%</span>
              </div>
            </div>
              ...
              <AnimatePresence mode="wait">
                {filteredWidgets.length === 0 ? (
                  ...
                  >
                    ...
                    <h3 className="text-lg font-medium mb-2">{translations.noResults}</h3>
                    <p className="text-gray-500 text-sm">{translations.noResultsDesc}</p>
                  </motion.div>
                ) : (
