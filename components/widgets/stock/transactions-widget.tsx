...
function TransactionRow({
  ...
  const totalAmount =
    transaction.type === 'sell' ? transaction.total : -transaction.total

  return (
    <TableRow className="group transition-colors hover:bg-gray-50 dark:hover:bg-gray-800/50">
...
export function TransactionsWidget({
  ...
  const summaryStats = useMemo(() => {
    const stats = {
      ...
      totalBuys: filteredAndSortedTransactions.filter(t => t.type === 'buy')
        .length,
      totalSells: filteredAndSortedTransactions.filter(t => t.type === 'sell')
        .length,
      totalDividends: filteredAndSortedTransactions.filter(
        t => t.type === 'dividend'
      ).length,
      totalValue: filteredAndSortedTransactions.reduce(
        (sum, t) => sum + Math.abs(t.total),
        0
      ),
      totalFees: filteredAndSortedTransactions.reduce(
        (sum, t) => sum + t.fees,
        0
      ),
    }
    ...
    >
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between border-t border-gray-200 pt-4 text-sm text-gray-600 dark:border-gray-700 dark:text-gray-400">
          <span>
            Viser {filteredAndSortedTransactions.length} av{' '}
