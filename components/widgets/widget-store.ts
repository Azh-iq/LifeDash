    ...
    persist(
      subscribeWithSelector(
        immer((set, get) => ({
          ...
          createLayout: (layoutId: string, widgets: WidgetInstance[] = []) =>
            set(state => {
            ...
            set(state => {
              ...
              if (state.activeLayoutId === layoutId) {
                ...
                state.activeLayoutId =
                  remainingLayouts.length > 0 ? remainingLayouts[0] : null
              }
            }),

          duplicateLayout: (sourceLayoutId: string, targetLayoutId: string) =>
            set(state => {
          ...
          // Widget management
          addWidget: (layoutId: string, widget: WidgetInstance) =>
            set(state => {
              if (!state.layouts[layoutId]) {
                state.layouts[layoutId] = []
              }
          ...
          removeWidget: (layoutId: string, widgetId: string) =>
            set(state => {
              if (state.layouts[layoutId]) {
                state.layouts[layoutId] = state.layouts[layoutId].filter(
                  w => w.id !== widgetId
                )
          ...
          updateWidget: (
            ...
            set(state => {
              if (state.layouts[layoutId]) {
                const widget = state.layouts[layoutId].find(
                  w => w.id === widgetId
                )
                if (widget) {
          ...
          selectMultipleWidgets: (widgetIds: string[]) =>
            set(state => {
              state.selectedWidgets = [
                ...new Set([...state.selectedWidgets, ...widgetIds]),
              ]
            }),
          ...
          // Drag and drop
          startDrag: (widgetId: string, dragData: WidgetDragData) =>
            set(state => {
              state.draggedWidget = widgetId
            }),

          updateDragTargets: (targets: WidgetDropTarget[]) =>
            set(state => {
          ...
          // Configuration
          startConfiguration: (widgetId: string) =>
            set(state => {
          ...
          // Grid layout
          updateGridLayout: (layout: WidgetGridLayout) =>
            set(state => {
              state.gridLayout = layout
            }),

          recalculateGrid: (layoutId: string) =>
            set(state => {
          ...
          // Event system
          addEventListener: (event: string, handler: WidgetEventHandler) =>
            set(state => {
          ...
          removeEventListener: (event: string, handler: WidgetEventHandler) =>
            set(state => {
          ...
          getWidgetsByCategory: (
            ...
            return (
              get().layouts[layoutId]?.filter(w => w.category === category) ||
              []
            )
          },
          ...
          validateWidgetPosition: (
            ...
            const otherWidgets =
              state.layouts[layoutId]?.filter(w => w.id !== widgetId) || []
            for (const otherWidget of otherWidgets) {
              const {
                row: otherRow,
                column: otherColumn,
                rowSpan: otherRowSpan,
                columnSpan: otherColumnSpan,
              } = otherWidget.position

              // Check if rectangles overlap
              if (
                !(
                  row >= otherRow + otherRowSpan ||
          ...
          validateWidgetSize: (
            ...
            if (
              rowSpan < minGridSize.rows ||
              columnSpan < minGridSize.columns ||
              rowSpan > maxGridSize.rows ||
              columnSpan > maxGridSize.columns
            ) {
              return false
            }
...
export const useActiveLayout = () =>
  useWidgetStore(state => state.activeLayoutId)
export const useWidgets = (layoutId?: string) =>
...
export const useWidgetMetrics = (widgetId?: string) =>
  useWidgetStore(state => (widgetId ? state.metrics[widgetId] : state.metrics))
