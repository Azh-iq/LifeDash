...
export const ProgressRing = forwardRef<SVGSVGElement, ProgressRingProps>(
  (
    ...
    const { radius, circumference, center, viewBox } = getCircleProps(
      size,
      strokeWidth
    )
    ...
    // Build classes
    const svgClasses = cn('drop-shadow-sm', className)
 ...
 */
export const AllocationRing = forwardRef<
  SVGSVGElement,
  Omit<ProgressRingProps, 'showPercentage' | 'color'> & {
  ...
  return (
    ...
    >
      <div className="text-center">
        <div className="text-lg font-bold">{Math.round(allocation)}%</div>
        {target && (
          <div className="text-xs text-gray-500">Target: {target}%</div>
        )}
        {assetName && (
          <div className="mt-1 text-xs text-gray-600">{assetName}</div>
        )}
      </div>
    </ProgressRing>
  )
 ...
 */
export const PerformanceRing = forwardRef<
  SVGSVGElement,
  Omit<ProgressRingProps, 'max' | 'color'> & {
 ...
 */
export const GoalProgressRing = forwardRef<
  SVGSVGElement,
  Omit<ProgressRingProps, 'showValue'> & {
    current: number
    goal: number
    currency?: string
  }
>(({ current, goal, currency = 'NOK', value, ...props }, ref) => {
  ...
  return (
    <ProgressRing ref={ref} value={progress} color={color} {...props}>
      <div className="text-center">
        <div className="text-base font-bold">{formatCurrency(current)}</div>
        <div className="text-xs text-gray-500">av {formatCurrency(goal)}</div>
        <div
          className={cn(
 ...
 */
export const MultiRingProgress = forwardRef<
  HTMLDivElement,
  {
    rings: Array<{
      value: number
...
>(({ rings, size = 120, spacing = 12, className }, ref) => {
