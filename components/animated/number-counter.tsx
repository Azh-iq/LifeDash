...
function formatNumber(
  ...
  const { currency, locale = 'nb-NO', decimals, prefix = '', suffix = '' } = options
...
export const NumberCounter = forwardRef<HTMLSpanElement, NumberCounterProps>(({
  ...
  const transformedValue = useTransform(motionValue, (latest) => {
    const formatted = formatNumber(latest, { currency, locale, decimals, prefix, suffix })
    return formatted
  })
  
  // Determine change characteristics
  const changeType = getChangeType(value, previousValue)
  const percentageChange = previousValue ? getPercentageChange(value, previousValue) : 0
  ...
  }, [value, motionValue, animationDuration, delay, ease, enableFlash, hasChanged, changeType, onUpdate, onComplete, isAnimating])
 ...
 */
export const CurrencyCounter = forwardRef<HTMLSpanElement, Omit<NumberCounterProps, 'currency'> & {
  currency?: string
  compact?: boolean
}>(({
  currency = 'NOK',
 ...
 */
export const PercentageCounter = forwardRef<HTMLSpanElement, Omit<NumberCounterProps, 'suffix' | 'decimals'> & {
  precision?: number
  showSign?: boolean
}>(({
  precision = 2,
 ...
 */
export const StockPriceCounter = forwardRef<HTMLSpanElement, Omit<NumberCounterProps, 'decimals'> & {
  tickSize?: number
}>(({
  tickSize = 0.01,
  value,
 ...
 */
export const PortfolioValueCounter = forwardRef<HTMLSpanElement, Omit<NumberCounterProps, 'size' | 'weight'> & {
  emphasis?: boolean
}>(({
  emphasis = false,
  ...props
