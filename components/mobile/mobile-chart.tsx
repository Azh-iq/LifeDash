...
const MobileChart = memo(({
  ...
  const drawChart = useCallback(() => {
    ...
    gradient.addColorStop(0, isPositive ? 'rgba(34, 197, 94, 0.1)' : 'rgba(239, 68, 68, 0.1)');
    ...
    chartData.forEach((point, index) => {
      ...
      const y = rect.height - padding - ((point.value - minValue) / valueRange) * chartHeight;
    ...
    if (selectedPoint) {
      const selectedIndex = chartData.findIndex(p => p.timestamp === selectedPoint.timestamp);
      if (selectedIndex !== -1) {
        const x = padding + (selectedIndex / (chartData.length - 1)) * chartWidth;
        const y = rect.height - padding - ((selectedPoint.value - minValue) / valueRange) * chartHeight;
    ...
    ctx.restore();
  }, [chartData, minValue, maxValue, valueRange, isPositive, zoom, pan, selectedPoint]);
  ...
  const handleTouchStart = useCallback((e: React.TouchEvent) => {
    ...
    if (e.touches.length === 1) {
      ...
      const padding = 20;
      const chartWidth = rect.width - 2 * padding;
      const dataIndex = Math.round(((x - padding) / chartWidth) * (chartData.length - 1));
  ...
  return (
    <Card className={cn('p-4 touch-none', className)}>
