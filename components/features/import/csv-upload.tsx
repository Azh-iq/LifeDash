...
import { Upload, FileText, X, AlertTriangle, CheckCircle, Info } from 'lucide-react'
    ...
    <div className={cn('w-full', className)}>
      ...
      <motion.div
        className={cn(
          ...
          !disabled && 'hover:border-gray-400 dark:hover:border-gray-600 cursor-pointer'
        )}
            ...
            {uploadState.isUploading ? (
              ...
              >
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-gray-500">
                    Drag and drop your Nordnet CSV export file here, or click to browse
                  </p>
        ...
        >
            ...
            <Alert>
              ...
              <AlertDescription>
                <div className="space-y-1">
                  ...
                  <ul className="text-sm space-y-1">
                    {uploadState.validationResult.warnings.map((warning, index) => (
                      <li key={index} className="list-disc list-inside">
                        {warning}
                      </li>
                    ))}
          ...
          {uploadState.validationResult.isValid && uploadState.parseResult && (
            <Alert>
              ...
              <AlertDescription>
                <div className="space-y-2">
                  <p className="font-medium">CSV file processed successfully!</p>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm">
                    <div>
                      <span className="font-medium">Transactions:</span> {uploadState.parseResult.totalRows}
                    </div>
                    <div>
                      <span className="font-medium">Portfolios:</span> {uploadState.parseResult.portfolios.length}
                    </div>
                    <div>
                      <span className="font-medium">Currencies:</span> {uploadState.parseResult.currencies.length}
                    </div>
                    <div>
                      <span className="font-medium">Stocks:</span> {uploadState.parseResult.isinCodes.length}
                    </div>
                  </div>
                </div>
              </AlertDescription>
        ...
        >
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Detected {uploadState.parseResult.totalRows} transactions from {uploadState.parseResult.portfolios.length} portfolios
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              {uploadState.parseResult.portfolios.length > 0 && (
                <div>
                  <h4 className="font-medium text-sm mb-2">Portfolios Found:</h4>
                  <div className="flex flex-wrap gap-1">
                    {uploadState.parseResult.portfolios.map((portfolio, index) => (
              ...
              {uploadState.parseResult.transactionTypes.length > 0 && (
                <div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {uploadState.parseResult.transactionTypes.map((type, index) => (
              ...
              {uploadState.parseResult.currencies.length > 0 && (
                <div>
                  <h4 className="font-medium text-sm mb-2">Currencies:</h4>
                  <div className="flex flex-wrap gap-1">
                    {uploadState.parseResult.currencies.map((currency, index) => (
                      <Badge key={index} variant="outline" className="text-xs">
                        {currency}
                      </Badge>
                    ))}
              ...
              <div className="pt-2 border-t text-xs text-gray-500 space-y-1">
                ...
                <div>Delimiter: "{uploadState.parseResult.detectedDelimiter}"</div>
                <div>Norwegian Characters: {uploadState.parseResult.hasNorwegianCharacters ? 'Yes' : 'No'}</div>
              </div>
            </CardContent>
          </Card>
        </motion.div>
