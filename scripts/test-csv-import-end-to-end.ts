...
async function testCSVImportEndToEnd() {
  ...
  try {
    ...
    console.log(
      `Testing import with ${limitedParseResult.totalRows} transactions...`
    )
    ...
    if (importResult.success && importResult.data) {
      ...
      if (
        ...
        importResult.data.processedData.length > 0
      ) {
        console.log('\n📋 Data Analysis:')
        ...
        importResult.data.processedData.forEach(transaction => {
          if (transaction.portfolio_name)
            portfolios.add(transaction.portfolio_name)
          if (transaction.currency) currencies.add(transaction.currency)
          if (transaction.internal_transaction_type) {
            const count =
              transactionTypes.get(transaction.internal_transaction_type) || 0
            transactionTypes.set(
              transaction.internal_transaction_type,
              count + 1
            )
          }
        })
    ...
    if (importResult.success) {
      ...
      if (parseResult.totalRows > 5) {
        console.log(
          `💡 Ready to import all ${parseResult.totalRows} transactions`
        )
