...
async function debugCSVImportSimple() {
  ...
  try {
    ...
    const requiredHeaders = ['Id', 'Bokføringsdag', 'Portefølje', 'Transaksjonstype', 'Beløp', 'Valuta']
    const missingHeaders = requiredHeaders.filter(h => !parseResult.headers.includes(h))
    ...
    for (const [index, row] of testRows.entries()) {
      try {
        ...
        console.log(`      Type: ${transactionData.transaction_type} → ${transactionData.internal_transaction_type}`)
        ...
        console.log(`      Needs Stock Lookup: ${transactionData.needs_stock_lookup}`)
        
        // Show validation results
        if (transactionData.validation_errors.length > 0) {
          console.log(`      ❌ Validation Errors: ${transactionData.validation_errors.join(', ')}`)
        }
        if (transactionData.validation_warnings.length > 0) {
          console.log(`      ⚠️  Validation Warnings: ${transactionData.validation_warnings.join(', ')}`)
        }
    ...
    console.log(`✅ Successfully transformed ${transformedTransactions.length}/${testRows.length} test rows`)
    ...
    if (transformedTransactions.length > 0) {
      ...
      transformedTransactions.forEach(t => {
        if (t.internal_transaction_type) {
          transactionTypes.set(t.internal_transaction_type, (transactionTypes.get(t.internal_transaction_type) || 0) + 1)
        }
      ...
      console.log(`   Transaction Types: ${Array.from(transactionTypes.keys()).join(', ')}`)
    ...
    const requiredDbFields = [
      'user_id', 'account_id', 'transaction_type', 'date', 'total_amount', 
      'currency', 'commission', 'other_fees', 'exchange_rate', 
    ...
    const optionalDbFields = [
      'stock_id', 'external_id', 'quantity', 'price', 'settlement_date',
      'description', 'notes'
    ]
    ...
    if (transformedTransactions.length > 0) {
      ...
      const fieldMappings = {
        ...
        'other_fees': (firstTransaction.total_fees || 0) - (firstTransaction.commission || 0),
        'currency': firstTransaction.currency,
        'exchange_rate': firstTransaction.exchange_rate || 1.0,
        'description': firstTransaction.transaction_text || firstTransaction.security_name,
    ...
    if (transformErrors.length === 0) {
      console.log('✅ No critical issues found in CSV parsing and transformation')
      ...
      console.log('   Fix transformation errors before proceeding to database operations')
    }
