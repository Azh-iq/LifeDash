  ...
  try {
    ...
    try {
      ...
      console.log(
        ...
        envFile.includes('NEXT_PUBLIC_SUPABASE_URL')
      )
      console.log(
        'Contains SUPABASE_KEY:',
        envFile.includes('NEXT_PUBLIC_SUPABASE_ANON_KEY')
      )
    } catch (e) {
    ...
    try {
      ...
      console.log(
        'First row keys:',
        parseResult.rows?.[0]
          ? Object.keys(parseResult.rows[0]).slice(0, 5)
          : 'No rows'
      )
    } catch (e) {
    ...
    try {
      const { importNordnetTransactions } = await import(
        '../lib/actions/transactions/csv-import'
      )
    ...
    console.log('\n🎯 NEXT STEPS:')
    console.log(
      '1. Check if development server is running on http://localhost:3005'
    )
