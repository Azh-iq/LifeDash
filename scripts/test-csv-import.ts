...
async function testCSVImport() {
  ...
  try {
    ...
    const fileStats = await import('fs').then(fs => fs.promises.stat(CSV_FILE_PATH))
    ...
    const norwegianHeaders = parseResult.headers.filter(header => 
      /[æøåÆØÅ]/.test(header) || 
      ['Bokføringsdag', 'Handelsdag', 'Oppgjørsdag', 'Portefølje', 'Transaksjonstype', 'Verdipapir', 'Kjøpsverdi', 'Beløp'].includes(header)
    )
    ...
    if (parseResult.rows.length > 0) {
      ...
      try {
        ...
        console.log(`❌ Field mapping failed: ${error instanceof Error ? error.message : 'Unknown error'}`)
      }
    }
