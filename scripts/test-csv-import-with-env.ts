...
async function testCSVImportWithEnvironment() {
  ...
  try {
    ...
    try {
      ...
      console.log(`Import completed: ${importResult.success ? 'SUCCESS' : 'FAILED'}`)
      ...
      if (importResult.success && importResult.data) {
        ...
        if (importResult.error?.includes('supabaseUrl is required')) {
          ...
          console.log('   Environment variables are not properly loaded in server action context')
          ...
          console.log('   Data violates database constraints or foreign key references')
        } else {
          console.log('\n💡 Diagnosis: Unknown database error')
          console.log('   Check Supabase logs and database schema compatibility')
        }
      }
    } catch (importError) {
      console.log(`❌ Import failed with exception: ${importError instanceof Error ? importError.message : 'Unknown error'}`)
    ...
    console.log('\n📋 Summary')
    console.log('===========')
    console.log('This test helps identify whether the CSV import issue is caused by:')
