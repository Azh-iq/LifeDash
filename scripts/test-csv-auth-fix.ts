...
async function testCSVAuthFix() {
  ...
  try {
    ...
    console.log('\n🧪 Test 1: Missing access token')
    let result = await importNordnetTransactions(parseResult, '', 'fake-user-id')
    ...
    result = await importNordnetTransactions(parseResult, 'invalid-token', 'fake-user-id')
    console.log('Result:', result.success ? '✅ Success' : '❌ Failed:', result.error)
    
    // Test 4: Check environment variables
    console.log('\n🧪 Test 4: Environment variables')
    console.log('SUPABASE_URL available:', !!process.env.NEXT_PUBLIC_SUPABASE_URL)
    console.log('SUPABASE_KEY available:', !!process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY)
