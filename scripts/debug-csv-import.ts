...
import { NordnetCSVParser, NordnetFieldMapper, NordnetTransactionTransformer } from '../lib/integrations/nordnet'
...
async function debugCSVImport() {
  ...
  try {
    ...
    const { data: testData, error: testError } = await supabase.from('user_profiles').select('id').limit(1)
    if (testError) {
    ...
    for (const row of parseResult.rows.slice(0, 5)) { // Test first 5 rows
      try {
    ...
    if (transformedTransactions.length > 0) {
      ...
      console.log(`   Internal Type: ${firstTransaction.internal_transaction_type}`)
      ...
      console.log(`   Needs Stock Lookup: ${firstTransaction.needs_stock_lookup}`)
      if (firstTransaction.validation_errors.length > 0) {
        console.log(`   Validation Errors: ${firstTransaction.validation_errors.join(', ')}`)
      }
      if (firstTransaction.validation_warnings.length > 0) {
        console.log(`   Validation Warnings: ${firstTransaction.validation_warnings.join(', ')}`)
      }
    }
    ...
    const testPortfolios = [...new Set(transformedTransactions.map(t => t.portfolio_name))]
    
    try {
      // Test account creation manually
      for (const portfolioName of testPortfolios.slice(0, 1)) { // Test first portfolio
        ...
        if (existingAccount) {
          ...
          const { data: existingPortfolio, error: portfolioFindError } = await supabase
            .from('portfolios')
            .select('id')
            .eq('user_id', user.id)
            .eq('name', portfolioName)
          ...
          if (portfolioFindError && portfolioFindError.code !== 'PGRST116') {
            console.log(`❌ Portfolio lookup failed: ${portfolioFindError.message}`)
            continue
          }
          ...
          if (createAccountError) {
            ...
            console.log(`   Data attempted: ${JSON.stringify(accountRecord, null, 2)}`)
            continue
          }
      ...
      console.log(`❌ Account creation test failed: ${error instanceof Error ? error.message : 'Unknown error'}`)
    ...
    const stockTransactions = transformedTransactions.filter(t => t.needs_stock_lookup && t.isin)
    const uniqueStocks = new Map()
    
    for (const transaction of stockTransactions.slice(0, 2)) { // Test first 2 stocks
      ...
      try {
        ...
        if (existingStock) {
          ...
          if (createError) {
            ...
            console.log(`   Data attempted: ${JSON.stringify(stockRecord, null, 2)}`)
            continue
          }
        ...
        console.log(`❌ Stock test failed: ${error instanceof Error ? error.message : 'Unknown error'}`)
      }
    }
    ...
    try {
      ...
      if (insertError) {
        ...
        console.log(`   Data attempted: ${JSON.stringify(testTransaction, null, 2)}`)
      } else {
      ...
      console.log(`❌ Transaction insert test failed: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
    ...
    try {
      ...
      console.log(`❌ Full import test failed: ${error instanceof Error ? error.message : 'Unknown error'}`)
      console.log(`   Full error: ${JSON.stringify(error, null, 2)}`)
    }
