  ...
  try {
    ...
    if (!user) {
      ...
      if (!testStockId) {
        ...
        console.log(
          `✅ Using existing stock: ${testStockId} (${existingStock.symbol})`
        )
      }
      ...
      if (createTransactionError) {
        console.log(
          `❌ Transaction creation failed: ${createTransactionError.message}`
        )
        ...
        if (createTransactionError.code === '23505') {
          console.log(
            '   💡 Duplicate key violation - transaction may already exist'
          )
        } else if (createTransactionError.code === '23503') {
          console.log(
            '   💡 Foreign key violation - referenced record may not exist'
          )
        } else if (createTransactionError.code === '23514') {
          console.log(
            '   💡 Check constraint violation - data may not meet validation rules'
          )
        }
      ...
      if (deleteTransactionError) {
        console.log(
          `⚠️  Failed to cleanup transaction: ${deleteTransactionError.message}`
        )
      } else {
