  ...
  try {
    ...
    for (let i = 0; i < Math.min(10, parseResult.rows.length); i++) {
      ...
      try {
        ...
        console.log(`   Date: "${transformed.booking_date}" (type: ${typeof transformed.booking_date})`)
        console.log(`   Type: "${transformed.transaction_type}" → "${transformed.internal_transaction_type}"`)
        console.log(`   Currency: "${transformed.currency}"`)
        console.log(`   Amount: ${transformed.amount} (type: ${typeof transformed.amount})`)
        ...
        if (!transformed.booking_date) {
          ...
          issues.push(`Transaction ${row.Id}: booking_date format incorrect: "${transformed.booking_date}"`)
        }
        
        if (!transformed.internal_transaction_type) {
          issues.push(`Transaction ${row.Id}: Missing internal_transaction_type`)
        }
        
        if (!transformed.currency || typeof transformed.currency !== 'string') {
          issues.push(`Transaction ${row.Id}: Invalid currency: "${transformed.currency}"`)
        }
        ...
        if (transformed.amount === null || transformed.amount === undefined) {
          ...
          issues.push(`Transaction ${row.Id}: Amount is not a number: ${transformed.amount} (${typeof transformed.amount})`)
        }
        ...
        if (transformed.validation_errors.length > 0) {
          issues.push(`Transaction ${row.Id}: Validation errors: ${transformed.validation_errors.join(', ')}`)
        }
        
      } catch (error) {
        issues.push(`Transaction ${row.Id}: Transformation failed: ${error instanceof Error ? error.message : 'Unknown error'}`)
      }
    }
    ...
    const validTransactionTypes = [
      'BUY', 'SELL', 'DIVIDEND', 'SPLIT', 'MERGER', 'SPINOFF',
      'DEPOSIT', 'WITHDRAWAL', 'FEE', 'INTEREST', 'TAX',
      'TRANSFER_IN', 'TRANSFER_OUT', 'REINVESTMENT'
    ]
    ...
    Array.from(transactionTypes).forEach(type => {
      ...
      if (!isValid) {
        issues.push(`Invalid transaction type: "${type}" (not in database enum)`)
      }
    })
    ...
    if (sampleTransactions.length > 0) {
      ...
      const mockStockId = transaction.needs_stock_lookup ? 'stock_789' : undefined
      ...
      const transactionRecord = {
        ...
        other_fees: Math.max(0, (transaction.total_fees || 0) - (transaction.commission || 0)),
      ...
      if (!transactionRecord.transaction_type) recordIssues.push('Missing transaction_type')
      if (!transactionRecord.date) recordIssues.push('Missing date')
      if (transactionRecord.total_amount === null || transactionRecord.total_amount === undefined) {
        recordIssues.push('Missing total_amount')
      }
      if (!transactionRecord.currency) recordIssues.push('Missing currency')
      
      // Check data types
      if (typeof transactionRecord.user_id !== 'string') recordIssues.push('user_id must be string')
      if (typeof transactionRecord.account_id !== 'string') recordIssues.push('account_id must be string')
      if (typeof transactionRecord.transaction_type !== 'string') recordIssues.push('transaction_type must be string')
      if (typeof transactionRecord.date !== 'string') recordIssues.push('date must be string')
      if (typeof transactionRecord.total_amount !== 'number') recordIssues.push('total_amount must be number')
      if (typeof transactionRecord.currency !== 'string') recordIssues.push('currency must be string')
      
      // Check constraints
      if (transactionRecord.commission < 0) recordIssues.push('commission cannot be negative')
      if (transactionRecord.other_fees < 0) recordIssues.push('other_fees cannot be negative')
      if (transactionRecord.exchange_rate <= 0) recordIssues.push('exchange_rate must be positive')
    ...
    if (issues.length === 0) {
      ...
      const typeIssues = issues.filter(i => i.includes('transaction_type') || i.includes('Invalid transaction type'))
      const currencyIssues = issues.filter(i => i.includes('currency') || i.includes('Invalid currency'))
      const amountIssues = issues.filter(i => i.includes('amount'))
      const validationIssues = issues.filter(i => i.includes('Validation errors'))
      
      if (dateIssues.length > 0) {
        console.log('\n📅 Date Format Issues:')
        console.log('   - Fix date parsing in NordnetFieldMapper.parseNordnetDate()')
      ...
      if (typeIssues.length > 0) {
        ...
        console.log('   - Ensure all Nordnet types map to valid database enum values')
      ...
      if (amountIssues.length > 0) {
        console.log('\n💰 Amount Issues:')
        console.log('   - Fix number parsing in NordnetFieldMapper.parseNordnetNumber()')
