...
async function testCSVParsingOnly() {
  ...
  try {
    ...
    console.log(
      `   Detected delimiter: "${parseResult.detectedDelimiter === '\t' ? 'TAB' : parseResult.detectedDelimiter}"`
    )
    console.log(
      `   Norwegian characters detected: ${parseResult.hasNorwegianCharacters ? 'YES ✅' : 'NO ❌'}`
    )
    ...
    for (let i = 0; i < mappingTestRows; i++) {
      ...
      try {
        ...
        console.log(
          `   Mapped type: "${transformed.internal_transaction_type}"`
        )
    ...
    Array.from(transactionTypes)
      .sort()
      .forEach(type => {
        const count = parseResult.rows.filter(
          row => row.Transaksjonstype === type
        ).length
    ...
    Array.from(portfolios)
      ...
      .forEach(portfolio => {
        const count = parseResult.rows.filter(
          row => row.Portefølje === portfolio
        ).length
        console.log(`   "${portfolio}": ${count} transactions`)
      })

    console.log(`\nSecurities (${securities.size}):`)
    Array.from(securities)
      .sort()
      .slice(0, 10)
      .forEach(security => {
        const count = parseResult.rows.filter(
          row => row.Verdipapir === security
        ).length
    ...
    if (norwegianCharRows.length > 0) {
      ...
      norwegianCharRows.slice(0, 3).forEach((row, index) => {
        const fields = Object.entries(row).filter(
          ([key, value]) => typeof value === 'string' && /[æøåÆØÅ]/.test(value)
        )
    ...
    console.log(
      `   - Delimiter: ${parseResult.detectedDelimiter === '\t' ? 'TAB character' : parseResult.detectedDelimiter}`
    )
    console.log(
      `   - Norwegian characters: ${parseResult.hasNorwegianCharacters ? 'Properly decoded' : 'Not detected'}`
    )
    ...
    console.log(
      `   2. The CSV import modal will handle the database operations`
    )
    console.log(
      `   3. Monitor import progress for any database-specific issues`
    )
  } catch (error) {
