...
class ChartCache {
  ...
  private async fetchPortfolioChartData(
    ...
    try {
      const { data, error } = await supabase.rpc('get_portfolio_chart_data', {
  ...
  private compressChartData(chartData: ChartData): ChartData {
    ...
    const compressedData = data.filter(
      (_, index) => index % compressionRatio === 0
    )
  ...
  getCacheStats(): {
    ...
    return {
      ...
      totalDataPoints,
      avgDataPointsPerChart:
        this.cache.size > 0 ? totalDataPoints / this.cache.size : 0,
...
export const useChartCache = () => {
  return {
    ...
    refreshPortfolioChartData:
      chartCache.refreshPortfolioChartData.bind(chartCache),
