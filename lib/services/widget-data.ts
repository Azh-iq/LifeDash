...
export interface WidgetDataService {
  ...
  getCompanyNews: (
    symbol: string,
    days?: number
  ) => Promise<WidgetCompanyNewsResult>

  // Portfolio data
  getPortfolioOverview: (
    portfolioId: string
  ) => Promise<WidgetPortfolioOverviewResult>
  getPortfolioHoldings: (
    portfolioId: string
  ) => Promise<WidgetPortfolioHoldingsResult>
  getPortfolioPerformance: (
    portfolioId: string
  ) => Promise<WidgetPortfolioPerformanceResult>
  getPortfolioTransactions: (
    portfolioId: string,
    limit?: number
  ) => Promise<WidgetTransactionsResult>

  // Analytics data
  getPortfolioAnalytics: (
    portfolioId: string
  ) => Promise<WidgetPortfolioAnalyticsResult>
...
class WidgetDataCache {
  ...
  getStats(): WidgetCacheStats {
    ...
    const hitRate =
      this.hitStats.hits / (this.hitStats.hits + this.hitStats.misses) || 0
    ...
    return {
      ...
      hitRate,
      totalSize,
      maxSize: Object.values(CACHE_CONFIGS).reduce(
        (sum, config) => sum + config.maxSize,
        0
      ),
      oldestEntry: oldest,
      newestEntry: newest,
...
class WidgetDataServiceImpl implements WidgetDataService {
  ...
  async getBasicFinancials(
    symbol: string
  ): Promise<WidgetBasicFinancialsResult> {
  ...
  async getPortfolioPerformance(
    ...
    try {
      ...
      holdings.forEach(holding => {
        ...
        sectorMap.set(
          sector,
          (sectorMap.get(sector) || 0) + holding.current_value
        )
      })

      const sectorAllocation = Array.from(sectorMap.entries()).map(
        ([sector, value]) => ({
          sector,
          value,
      ...
      holdings.forEach(holding => {
        ...
        currencyMap.set(
          currency,
          (currencyMap.get(currency) || 0) + holding.current_value
        )
      })

      const currencyAllocation = Array.from(currencyMap.entries()).map(
        ([currency, value]) => ({
          currency,
          value,
          percent: totalValue > 0 ? (value / totalValue) * 100 : 0,
      ...
      const topPerformers = holdings
        ...
        .map(holding => ({
          ...
          contribution:
            totalValue > 0 ? (holding.gain_loss / totalValue) * 100 : 0,
        }))
    ...
    try {
      ...
      const filteredTransactions =
        transactionsData?.filter(
          (transaction: any) =>
            transaction.accounts?.portfolio_id === portfolioId
        ) || []

      const hasMore = filteredTransactions.length > limit
      const transactions = filteredTransactions.slice(0, limit)

      const widgetTransactions: WidgetTransaction[] = transactions.map(
        (transaction: any) => ({
          id: transaction.id,
          portfolioId,
